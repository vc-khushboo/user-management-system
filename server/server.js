...
function isAuthenticated({username, password}){
  ...
  return userss.users.find(user => user.username == username && user.password == password)
}
...
server.post('/user/add', (req, res) => {
  ...
  fs.readFile("./server/users.json", (err, data) => {
    ...
    //Add new user
    data.users.push({id: last_item_id + 1, username: username, password: password, userrole: '2', age: age, address: address, phone: phone, name: name}); //add some data
...
server.post('/auth/login', (req, res) => {
  ...
  res.status(200).json({ token :access_token, user:{ username: user.username, userrole: user.userrole, id: user.id }});
})
...
server.put('/user/edit', (req, res) => {
  ...
  fs.readFile("./server/users.json", (err, data) => {  
    ...
    //Add new user
    var writeData = fs.writeFile("./server/users.json", JSON.stringify(data), (err, result) => { 
        if (err) {
          const status = 401
          const message = err
          res.status(status).json({status, message})
